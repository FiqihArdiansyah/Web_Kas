...
function addmahasiswa($data) {
	...
	$query = mysqli_query($conn, "INSERT INTO mahasiswa VALUES ('', '$nama_mahasiswa', '$jenis_kelamin', '$no_telepon', '$email')");
  	return mysqli_affected_rows($conn);
}
...
function editmahasiswa($data) {
	...
	$no_telepon = htmlspecialchars($data['no_telepon']);
	$email = htmlspecialchars($data['email']);
	$query = mysqli_query($conn, "UPDATE mahasiswa SET nama_mahasiswa = '$nama_mahasiswa', jenis_kelamin = '$jenis_kelamin', no_telepon = '$no_telepon', email = '$email' WHERE id_mahasiswa = '$id_mahasiswa'");
  	return mysqli_affected_rows($conn);
}
...
function addBulanPembayaran($data) {
	...
	if (mysqli_fetch_assoc($check_bulan)) {
		...
		$datamahasiswa = mysqli_query($conn, "SELECT * FROM mahasiswa");
		$mahasiswa = "INSERT INTO uang_kas (id_mahasiswa, id_bulan_pembayaran, minggu_ke_1, minggu_ke_2, minggu_ke_3, minggu_ke_4) VALUES ";
		foreach ($datamahasiswa as $ds) {
		 	$mahasiswa .= "('{$ds['id_mahasiswa']}','{$id_bulan_pembayaran}','','','','')";
            $mahasiswa .= ",";
		}
		$mahasiswa = rtrim($mahasiswa,",");
		$query_mahasiswa = mysqli_query($conn, $mahasiswa);
	  	return mysqli_affected_rows($conn);
	}
}
...
function tambahmahasiswaUangKas($data) {
	global $conn;
	$id_mahasiswa = htmlspecialchars($data['id_mahasiswa']);
	$id_bulan_pembayaran = htmlspecialchars($data['id_bulan_pembayaran']);
	$query = mysqli_query($conn, "INSERT INTO uang_kas VALUES ('', '$id_mahasiswa', '$id_bulan_pembayaran', '0', '0', '0', '0')");
  	return mysqli_affected_rows($conn);
}
