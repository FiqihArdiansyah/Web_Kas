<?php 
  ...
  checkLogin();
  $riwayat = mysqli_query($conn, "SELECT * FROM riwayat INNER JOIN user ON riwayat.id_user = user.id_user INNER JOIN uang_kas ON riwayat.id_uang_kas = uang_kas.id_uang_kas INNER JOIN mahasiswa ON uang_kas.id_mahasiswa = mahasiswa.id_mahasiswa INNER JOIN bulan_pembayaran ON uang_kas.id_bulan_pembayaran = bulan_pembayaran.id_bulan_pembayaran ORDER BY tanggal DESC");
?>
          ...
          <table class="table table-striped table-hover" id="table_id">
            ...
            <tbody>
              ...
              <?php foreach ($riwayat as $dr): ?>
                <tr>
                  <td><?= $i++; ?></td>
                  <td><?= ucwords($dr['nama_mahasiswa']); ?></td>
